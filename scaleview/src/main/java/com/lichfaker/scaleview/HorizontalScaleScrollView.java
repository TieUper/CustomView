...
public class HorizontalScaleScrollView extends BaseScaleView {
    ...
    protected void onDrawPointer(Canvas canvas, Paint paint) {
      ...
      //  int finalX = mScroller.getFinalX();
        ...
        //滑动的刻度
        int tmpCountScale = (int) Math.rint((double) mScrollLastX / (double) mScaleMargin); //四舍五入取整
        //总刻度
        //mCountScale = tmpCountScale + countScale + mMin;
        if (mScrollListener != null) { //回调方法
            mScrollListener.onScaleScroll(mCountScale);
        }
        canvas.drawLine(mScaleMargin + mScrollLastX, mRectHeight,
                mScaleMargin + mScrollLastX, mRectHeight - mScaleMaxHeight - mScaleHeight, paint);
    }
